...
export async function getPatchNotes() {
  ...
  const patchNotes = await resolveCommand(
    "git log --oneline --no-merges --no-decorate --no-color --pretty=format:'%h %s' --abbrev-commit --since='last week'"
  );
...
export async function getCLNotes() {
  ...
  const log = await resolveCommand(
    "git log --oneline --no-merges --no-decorate --no-color --pretty=format:'%h %s' --abbrev-commit --since='last week'"
  );
